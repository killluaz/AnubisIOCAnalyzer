...
class IOCAnalyzer:
    ...
    def install_prerequisites(self):
        ...
        try:
            ...
            print(f"{Fore.GREEN}[+] Instalación de prerrequisitos completada exitosamente.{Style.RESET_ALL}")
...
<body>
                                ...
                                {% if engine == 'virustotal' and data.last_analysis_stats %}
                                    <div class="detail-item">
                                        ...
                                        <span class="detail-value" style="color: {% if data.last_analysis_stats.malicious > 5 %}#e74c3c{% else %}#27ae60{% endif %};">
                                            {{ data.last_analysis_stats.malicious }}/{{ data.last_analysis_stats.malicious + data.last_analysis_stats.undetected }}
                                        </span>
                                    </div>
                                    <div class="detail-item">
    ...
    <script>
        ...
        (function() {
            ...
            {% if result.engines_results.virustotal.success and result.engines_results.virustotal.last_analysis_stats %}
            ...
            {% else %}
            // Gráfico de resumen general si no hay datos de VirusTotal
            new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: {{ result.engines_results.keys() | list | tojson }},
                    datasets: [{
                        label: 'Score de Detección',
                        data: [
                            {% for engine, data in result.engines_results.items() %}
                                {% if engine == 'virustotal' and data.success %}
                                    {{ ((data.last_analysis_stats.malicious | default(0)) / (data.last_analysis_stats.malicious | default(0) + data.last_analysis_stats.undetected | default(1))) * 100 if data.last_analysis_stats else 0 }}
                                {% elif engine == 'abuseipdb' and data.success %}
                                    {{ data.abuse_confidence_score | default(0) }}
                                {% elif engine == 'maltiverse' and data.success %}
                                    {{ data.threat_score | default(0) }}
                                {% else %}
                                    0
                                {% endif %}
